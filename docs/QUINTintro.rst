...
The QUINT workflow comprises a flexible suite of software designed to support atlas-based quantification of labelled features in series of histological images from mouse or rat brain. All the software have graphical user interfaces (GUIs), with no coding ability required. The QUINT workflow generates object counts and percentage coverage per atlas-region, in addition to point clouds for visualising the features in 3D atlas space. It currently supports quantification relative to the following atlases:
